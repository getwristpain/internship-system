...
<div class="flex flex-col gap-2 {{ $disabled ? 'opacity-100 cursor-not-allowed' : '' }}">
    @if ($label)
    <label for="{{ $id }}" class="flex text-sm font-medium text-neutral-600 {{ $required ? 'required' : '' }}">
        <span>{{ $label }}</span>
        @if ($help)
    ...
    <div class="flex items-center gap-2">
        <div class="relative {{ $width }}">
            <iconify-icon icon="{{ match ($custom) {
                    'email' => 'mdi:email',
            ...
            @if ($type === 'textarea')
            <textarea id="{{ $id }}" name="{{ $name }}" @if ($model) wire:model.live.debounce.1000ms="{{ $model }}"
                @endif placeholder="{{ $placeholder }}" autocomplete="{{ $name }}" {{ $attributes->merge([
                        'class' =>
            ...
            @elseif ($type === 'date')
            <input id="{{ $id }}" name="{{ $name }}" type="{{ $type }}" @if ($model) wire:model.live="{{ $model }}"
                @endif placeholder="{{ $placeholder }}" autocomplete="{{ $name }}" {{ $attributes->merge([
            'class' =>
            ...
            @else
            <input id="{{ $id }}" name="{{ $name }}" type="{{ $type }}" @if ($model)
                wire:model.live.debounce.1000ms="{{ $model }}" @endif placeholder="{{ $placeholder }}"
                autocomplete="{{ $name }}" {{ $attributes->merge([
            'class' =>
            'w-full pl-10 input input-bordered focus:outline-none focus:ring-2 focus:ring-neutral
            disabled:bg-neutral-100 disabled:cursor-not-allowed' .
            (empty($errorMessages) ? '' : ' border-red-500 focus:ring-red-500'),
